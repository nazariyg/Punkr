...
public enum MimeType: String, CaseIterable {
    ...
    public init?(fileExtension: String) {
        ...
        let foundCase = Self.allCases.first { $0.value.lowercased() == fileExtensionLowercased }
    ...
    private typealias `Self` = MimeType
    ...
    private static func defaultMimeType(forString mimeTypeString: String) -> Self? {
        let mimeTypeStringLowercased = mimeTypeString.lowercased()
        if Self.bin.value.lowercased() == mimeTypeStringLowercased { return Self.bin }  // "application/octet-stream" -> .bin
        return nil
    }
