...
public extension Reactive where Base: SessionManager {
    ...
    func making<ResponsePayload>(
        request: HTTPRequest, for _: ResponsePayload.Type, acceptForValidation: [HTTPContentType] = [])
        -> SignalProducer<HTTPResponse<ResponsePayload>, Error> {

        // Output injection.
        if let injectedOutputResult = InstanceProvider.shared.outputForInput(request, outputType: HTTPResponse<ResponsePayload>.self) {
            var signalProducer = SignalProducer<HTTPResponse<ResponsePayload>, Error>(value: injectedOutputResult.output)
        ...
        return
            SignalProducer { [weak base] emitter, lifetime in
                        ...
                        case let .success(value):
                            if let urlResponse = afResponse.response {
                                ...
                                guard let json = jsonValue else {
                                    let error: Error = .unexpectedHTTPResponsePayload
                                    log.error(error.description, logCategory)
                                    emitter.send(error: error)
                                    return
                                }
    ...
    func downloading(url: URL, acceptForValidation: [HTTPContentType] = []) -> SignalProducer<DownloadStatus, Error> {
        return
            SignalProducer { [weak base] emitter, lifetime in
                guard let sessionManager = base else {
                    emitter.sendInterrupted()
    ...
    func uploadingFile(withURL fileURL: URL, toURL url: URL) -> SignalProducer<UploadStatus, Error> {
        return
            SignalProducer { [weak base] emitter, lifetime in
                guard let sessionManager = base else {
                    emitter.sendInterrupted()
    ...
    func uploadingData(_ data: @autoclosure @escaping () -> Data, toURL url: URL) -> SignalProducer<UploadStatus, Error> {
        return
            SignalProducer { [weak base] emitter, lifetime in
                guard let sessionManager = base else {
                    emitter.sendInterrupted()
    ...
    func makingForData(request: HTTPRequest, acceptForValidation: [HTTPContentType] = []) -> SignalProducer<HTTPDataResponse, Error> {
    ...
    private static func processURLResponseCodeForErrors<ResponsePayload>(
        _ urlResponse: HTTPURLResponse?, error: Swift.Error?, emitter: Signal<ResponsePayload, Error>.Observer) -> Bool {
        ...
        if let statusCodeInt = urlResponse?.statusCode {
            ...
            if statusCode == .notFound && Config.shared.general.acknowledgeHTTPResponseNotFoundErrors {
                let error: Error = .networkResponseNotFound
                log.error(error.description, logCategory)
                emitter.send(error: error)
                return true
            }
        ...
        if let error = error as? AFError {
            if case .responseValidationFailed = error {
                let error: Error = .unexpectedHTTPResponseContentType
                log.error(error.description, logCategory)
                emitter.send(error: error)
                return true
            }
